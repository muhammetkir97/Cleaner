...
public class DustManager : MonoBehaviour
{
	...
	[SerializeField] private RobotCleaner robotCleaner;
	...
	void Update()
	{
		...
		dustCompute.SetVector("attractorPos", robotCleaner.transform.position);
		dustCompute.SetInt("numParticles", numParticles);
		dustCompute.SetFloat("size", size);
		dustCompute.SetVector("attractPos", robotCleaner.transform.position);
		dustCompute.SetVector("xAxis", robotCleaner.transform.right);
		dustCompute.SetFloat("attractRadius", attractRadius);
		dustCompute.SetFloat("attractForce", (robotCleaner.GetRobotStatus() == RobotStatus.Working) ? attractForce : 0);
